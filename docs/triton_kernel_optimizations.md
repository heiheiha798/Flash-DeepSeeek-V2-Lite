...
- 优化目标是 `A100 / SM80 / bf16` 上的 batched `q_len=1` decode sweep
...
当前保留两套显式 kernel family：small-GEMV family 和 grouped batching
family。`small` 是从 `bsz=1` 执行模板延展出来的 GEMV-oriented 路径，主要
代表 low-batch / `bsz=1` 优化方向；`batch` 是 grouped batching 路径，主
要代表大 batch 吞吐优化方向。它们由 `src/run.py --kernel-family` 显式选
择，不再通过环境变量或 batch-size 阈值自动切换。
...
- GPU：一张空闲 `NVIDIA A100 80GB PCIe / SM80`
- 场景：eager prefill + CUDA graph batched decode, `q_len=1`, batch sweep
  `1 2 4 8 16 32 64 128 256`, `max_new_tokens=100`
...
- 对比对象：gate/up 分离 dot 与 combined gate/up dot
...
不同物理 A100 的绝对速度会有差异，但 kernel 结构一致。这个 profile
用于确认当前优化空间已经主要集中在 MoE、GEMV/GEMM 和少量 torch
elementwise。
...
Validated on one idle NVIDIA A100 80GB PCIe GPU with input length 24 and
`max_new_tokens=100`:
...
| Batch | Path | Decode TPS |
| ---: | --- | ---: |
| 1 | `triton_batching_graph` | 158.07 |
| 2 | `triton_batching_graph` | 277.16 |
| 4 | `triton_batching_graph` | 537.06 |
| 8 | `triton_batching_graph` | 959.59 |
| 16 | `triton_batching_graph` | 1958.82 |
| 32 | `triton_batching_graph` | 3631.14 |
| 64 | `triton_batching_graph` | 5745.78 |
| 128 | `triton_batching_graph` | 7902.87 |
| 256 | `triton_batching_graph` | 9402.64 |
...
Validated on one idle A100 80GB PCIe GPU, `bsz=256`, fixed tile
...
End-to-end fixed-tile comparison on one idle A100 80GB PCIe GPU:
...
`9789.83` decode TPS on one idle A100 80GB PCIe GPU. The small TPS delta is close to run-to-run
...
A100 after this change. `bsz=128` also improved in the quick A/B check:
