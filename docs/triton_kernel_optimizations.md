...
Current `src/sota.py` keeps the original `batch_size=1` full SOTA path and adds a
separate `batched_cuda_graph` path for `batch_size>1`. The batched path captures
the decode loop with CUDA graph and uses a batched packed-routed-MoE Triton path;
it intentionally leaves the existing single-token MoE kernel untouched so bs=1
performance remains comparable.

Validated on RTX A6000 GPU2 with `max_new_tokens=100`:

| Batch | Path | Decode TPS | Log |
| ---: | --- | ---: | --- |
| 1 | `triton_sota_graph` | 135.97 | `/tmp/flashmla_sota_batched_graph_bs1_2_4.log` |
| 2 | `batched_cuda_graph` | 203.13 | `/tmp/flashmla_sota_batched_graph_bs1_2_4.log` |
| 4 | `batched_cuda_graph` | 347.99 | `/tmp/flashmla_sota_batched_graph_bs1_2_4.log` |
| 8 | `batched_cuda_graph` | 482.78 | `/tmp/flashmla_sota_batched_graph_bs8_16_32.log` |
| 16 | `batched_cuda_graph` | 610.88 | `/tmp/flashmla_sota_batched_graph_bs8_16_32.log` |
| 32 | `batched_cuda_graph` | 714.91 | `/tmp/flashmla_sota_batched_graph_bs8_16_32.log` |
| 64 | `batched_cuda_graph` | 779.40 | `/tmp/flashmla_sota_batched_graph_bs64.log` |

Remaining work: the batched path still uses the framework attention implementation
for `batch_size>1`; extending `attention_decode_only_triton` to true batched MLA
should be the next optimization target.
