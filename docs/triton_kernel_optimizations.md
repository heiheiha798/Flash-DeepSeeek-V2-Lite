...
Current `src/sota.py` uses a unified `triton_decode_graph` path for q_len=1
decode across batch sizes 1 through 256. The attention path now calls a shared
`attention_decode_triton(...)` implementation that packs q/kv-a, kv-b, RoPE,
cache write, decode attention, and o-proj into Triton kernels. The routed MoE
