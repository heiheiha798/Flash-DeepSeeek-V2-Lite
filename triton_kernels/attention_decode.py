...
def _cache_write_q1_dual_kernel(
    ...
    tl.store(
        k_cache_ptr + head_idx * k_cache_stride_h + pos * k_cache_stride_s + d_offsets * k_cache_stride_d,
        k,
        mask=k_mask,
    )
    ...
    v = tl.load(
        v_new_ptr + head_idx * v_new_stride_h + d_offsets * v_new_stride_d,
        mask=v_mask,
        other=0.0,
    )
    tl.store(
        v_cache_ptr + head_idx * v_cache_stride_h + pos * v_cache_stride_s + d_offsets * v_cache_stride_d,
        v,
        mask=v_mask,
    )
